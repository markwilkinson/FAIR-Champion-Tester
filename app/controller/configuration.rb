...
$tests_hash = {}
warn `pwd`
Dir['./configuration/*.conf'].each do |file|
  config = ParseConfig.new(file)
  #warn "config" + config.inspect.to_s
  id = config['metadata']['guid']
  $tests_hash[id] = Hash.new("")
  warn "id #{id}"
  %w[guid title classname operation_path tests_metric description applies_to_principle version organization org_url responsible_developer email
      developer_orcid response_description comments].each do |param|
    $tests_hash[id][param] = config['metadata'][param]
  end
end
  #warn $th.inspect
